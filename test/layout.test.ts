...
test("fitSegmentsToWidth compacts pet before dropping high-priority widgets", () => {
  const segments: Segment[] = [
    ...
    { id: "pet", text: "(>_<) fixing feature/demo", compactText: "(>_<)", tone: "warning", priority: 65 }
  ];
...
test("fitSegmentsToWidth drops low-priority widgets when compacting is not enough", () => {
  ...
  assert.equal(fitted.some((segment) => segment.id === "pet"), false);
  assert.equal(fitted.some((segment) => segment.id === "model"), true);
});
