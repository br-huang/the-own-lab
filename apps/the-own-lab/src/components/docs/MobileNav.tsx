        ...
        node.kind === 'link' ? (
          <li key={node.href}>
            <a
              ...
              className={`block px-3 py-2 rounded text-sm ${
                node.active ? 'docs-nav-link docs-nav-link-active' : 'docs-nav-link'
              }`}
            >
              {node.title}
            </a>
          ...
          <li key={node.title}>
            <details open={node.expanded}>
              <summary className="docs-nav-section-mobile">{node.title}</summary>
              <div className="pl-3">
...
export default function MobileNav({ sidebar }: Props) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="docs-mobile-panel" role="dialog" aria-label="Navigation menu">
          <nav>
