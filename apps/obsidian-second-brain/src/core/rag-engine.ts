...
export class RagEngine {
  ...
  async *query(userQuestion: string, forcedFiles?: string[]): AsyncGenerator<RagResponse> {
    ...
    if (!this.settings.openaiApiKey) {
      yield {
        type: 'error',
        message: 'Please configure your OpenAI API key in the plugin settings.',
      } as RagResponse;
      return;
    }
    ...
    if (await this.vectorStore.isEmpty()) {
      yield {
        type: 'error',
        message: 'No notes have been indexed yet. Please wait for indexing to complete.',
      } as RagResponse;
      return;
    }
      ...
      for (const fp of forcedFiles) {
        try {
          ...
          if (content.length > MAX_FORCED_FILE_SIZE) {
            content =
              content.substring(0, MAX_FORCED_FILE_SIZE) + '\n\n[Content truncated at 100 KB]';
          }
